...
def register():
    ...
    if age < 8:
        ...
        while len(c_n)<10 or len(c_n) > 10:
            ...
            contact_num = int(input("Enter your contact number."))
    user_name = (random.sample(set_fn,2) + random.sample(set_ln,2) + random.sample(set_pass,2) + random.sample(city,2))
    username1 = ''.join(user_name)
    print(f"Your Generated username is '{username1}'. You can use it for login and further purposes.")
...
print("Hey! Welcome to the quizz game. In order to play this game you first need to regoster then login.")
a = input('''Shall we proceed to register? (if yes then type YES and if no then type NO)''').upper()
time.sleep(1)
