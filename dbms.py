...
conn = mysql.connector.connect(
        host="localhost", 
        user="root",
        password="garenafreefire18#",
        database="garvitagrawal"
    )
cursor = conn.cursor()
...
def register():
    ...
    if age < 8:
        ...
        return  # Exit the function if the age is less than 8
    ...
    set_en = input("Enter your college enrollment number: ")
    ...
    sql = "INSERT INTO users(first_name,last_name,password,email,contact_number) values (%s,%s,%s,%s,%s)"
    val = (set_fn,set_ln,set_pass,set_email,contact_num)

    cursor.execute(sql,val)
    conn.commit()

    print("User registered successfully and data stored in the database!")
